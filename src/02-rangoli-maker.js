...
export function repeatPattern(pattern, times) {
  if(typeof pattern==="string" && Number.isInteger(times) && times>0) return pattern.repeat(times)
  else return ''
}

export function extractRangoliCenter(design, start, end) {
  if(typeof design==="string" && !isNaN(start) && !isNaN(end)) return design.slice(start, end)
    else return '' 
}
...
export function replaceRangoliColor(design, oldColor, newColor) {
  if(typeof design==="string" && typeof oldColor==="string" && typeof newColor==="string") return design.replaceAll(oldColor, newColor)
  else return ''
}

export function makeRangoliBorder(char, length) {
  if(typeof char==="string" && length>0) return char.repeat(10).slice(0, length)
    else return ''
}
